...
int gr_mesh_cmd(lua_State* L)
{
  ...
  if (num_args == 3) {
    ...
    std::vector<Point2D> uv_map;

    luaL_checktype(L, 2, LUA_TTABLE);
    int uv_count = luaL_getn(L, 2);

    luaL_argcheck(L, vert_count == uv_count, 2, "Expected uv coordinate for each vertex");
    ...
    Mesh* mesh = new Mesh(verts, faces, uv_map);
    GRLUA_DEBUG(*mesh);
    data->node = new GeometryNode(name, mesh);
  }
...
int gr_render_cmd(lua_State* L)
{
  ...
  project_render(root->node, filename, width, height,
            eye, view, up, fov,
            ambient, lights);
...
int gr_material_cmd(lua_State* L)
{
  ...
  if (num_args == 3) {
    // Phong Material
    data->material = new PhongMaterial(Colour(kd[0], kd[1], kd[2]), Colour(ks[0], ks[1], ks[2]),
        shininess);
  } else if (num_args == 6) {
    // Reflective/refractive material
    double reflection = luaL_checknumber(L, 4);
    double refraction = luaL_checknumber(L, 5);
    double refractive_index = luaL_checknumber(L, 6);
    data->material = new RMaterial(Colour(kd[0], kd[1], kd[2]), Colour(ks[0], ks[1], ks[2]),
        shininess, reflection, refraction, refractive_index);
  }
...
//  data->material = 0;
  ...
  if (num_args == 0) {
    // Sphere texture
    data->material = new SphereTexture();
  } else if (num_args == 1) {

    const char* file = luaL_checkstring(L, 1);
    data->material = new ImageTexture(file);
  } else if (num_args == 3) {
    const char* file = luaL_checkstring(L, 1);
    ...
    data->material = new PhongTexture(file, Colour(ks[0], ks[1], ks[2]), shininess, 0);
  } else if (num_args == 4) {
    const char* file = luaL_checkstring(L, 1);
    ...
    data->material = new PhongTexture(file, Colour(ks[0], ks[1], ks[2]), shininess, reflection);
  }
...
int gr_bump_cmd(lua_State* L)
{
  ...
  if (num_args == 4) {
    ...
    data->material = new BumpMaterial(file, Colour(kd[0], kd[1], kd[2]), Colour(ks[0], ks[1], ks[2]),
        shininess);
...
// Set a CSG node's operation
extern "C" int gr_node_set_operation_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
  ...
  luaL_argcheck(L, self != 0, 1, "CSG node expected");
...
static const luaL_reg grlib_functions[] = {
  ...
  {"cone", gr_cone_cmd},
  {"csg", gr_csg_cmd},
  {"s_texture", gr_texture_cmd},
  {"phong_texture", gr_texture_cmd},
  {"bump_material", gr_bump_cmd},
  {0, 0} // ???
};

// This is where all the lua_gettop(L)member functions for "gr.node" objects are
...
static const luaL_reg grlib_node_methods[] = {
  ...
  // DLM
  {"set_operation", gr_node_set_operation_cmd},
  {0, 0}
};
